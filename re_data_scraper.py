...
def deeper_info(re_dict):
    with webdriver.Firefox(executable_path='./geckodriver.exe') as dr:
        ...
        try:
            ...
            prop_tax = pandas.read_html(table.prettify())[1]
            print(prop_tax)
        except Exception as e:
            prop_hist = pandas.DataFrame()
        if not os.path.exists('./Property_history/'):
            os.makedirs('./Property_history/')
        prop_hist.to_csv("./Property_history/"+re_dict["Link"].split("/")[-1]+".csv")

        

def base_info(soup, re_list, page_counter):
    if page_counter!=1:
        with webdriver.Firefox(executable_path='./geckodriver.exe') as dr:
...
dr = webdriver.Firefox(executable_path='./geckodriver.exe')
...
while page_counter <= total_pages:
    base_info(soup, re_list, page_counter)
    page_counter+=1
